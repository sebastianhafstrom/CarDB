...
const downloadAllImages = async () => {
  for (const car of models) {
    if (car.imageUrl) {
      const fileName = `${car.brand}-${car.slug
        .replace(/\s+/g, "-")
        .toLowerCase()}.png`; // Customize the filename as needed
      console.log(`Downloading ${car.imageUrl} as ${fileName}`);
      await downloadImage(car.imageUrl, fileName);
    }
  }
